...
class Product extends CI_Controller
{
    ...
    function index()
    {
        $jum = $this->product_model->get_products();
        ...
        $x['data'] = $this->product_model->get_product_perpage($offset, $limit);
    ...
    {
        $data = $this->product_model->get_post_by_slug($slug);
        if ($data->num_rows() > 0) {
            $q = $data->row_array();
            $kode = $q['post_id'];
            ...
            $this->product_model->count_views($kode);
            $x['related_post']  = $this->product_model->get_related_post($category_id, $kode);
            $x['show_comments'] = $this->product_model->show_comments($kode);
            $site_info = $this->db->get('tbl_site', 1)->row();
            $v['logo'] =  $site_info->site_logo_header;
            $x['icon'] = $site_info->site_favicon;
            $x['header'] = $this->load->view('header', $v, TRUE);
    ...
    function submit_komentar()
    {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $this->product_model->save_comment($post_id, $name, $email, $comment);
