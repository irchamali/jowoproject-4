...
class Home_setting extends CI_Controller
{
	...
	function update()
	{
		...
		} elseif (!empty($_FILES['image_1']['name']) && empty($_FILES['image_2']['name']) && empty($_FILES['image_3']['name'])) {
			if ($this->upload->do_upload('image_1')) {
				$image_1 = $this->upload->data();
				$bg_1 = $image_1['file_name'];
			}
